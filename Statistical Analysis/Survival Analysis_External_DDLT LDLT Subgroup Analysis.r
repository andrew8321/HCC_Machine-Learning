...
# 2. Subset to LDLT group only and factorize risk group
df_ldlt <- df %>%
  filter(treatment == "LT", LDLT == 0) %>%
  mutate(risk_group = factor(risk_lt, levels = c("High Risk", "Low Risk")))  # Low Risk가 위로

# 3. Fit KM and Cox models
km_fit <- survfit(Surv(survival_time, event) ~ risk_group, data = df_ldlt)
cox_fit <- coxph(Surv(survival_time, event) ~ risk_group, data = df_ldlt)
...
# 6. Labels
p_text <- ifelse(p_value < 0.001, expression(italic(P) < ".001"), sprintf("P = %.3f", p_value))
hr_text <- sprintf("HR = %.2f (95%% CI: %.2f–%.2f)", hr, ci_lower, ci_upper)

legend_labels <- c(
  ifelse(is.na(mos_high), "High-Risk LDLT mOS=u.d.", sprintf("High-Risk LDLT mOS=%.0f", mos_high)),
  ifelse(is.na(mos_low), "Low-Risk LDLT mOS=u.d.", sprintf("Low-Risk LDLT mOS=%.0f", mos_low))
)
...
ggsave("KMplot_LDLT_risk_group_external.png", plot = combined_final_plot, width = 10, height = 12, dpi = 300)
...
print(combined_final_plot)
ggsave("KMplot_DDLT_risk_group_external.png", plot = combined_final_plot, width = 10, height = 12, dpi = 300)
