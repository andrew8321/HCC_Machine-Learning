...
df = pd.read_excel(LT_DATA_PATH, sheet_name="Sheet1")
...
X_train, X_test, y_train, y_test, scaler, num_cols, _ = split_and_scale(
    df, CATEGORICAL_COLS, TARGET_COL,
    test_size=TEST_SIZE,
    random_state=TRAIN_TEST_SPLIT_SEED,
    scale_full_dataset=True,
)

joblib.dump(scaler, MODELS_DIR / "scaler_LT.pkl")

spw = pos_weight(y_train)
models_dict = build_models(spw, LT_XGB_PARAMS, LT_RF_N_ESTIMATORS)
train_models(models_dict, X_train, y_train)

for name, model in models_dict.items():
    joblib.dump(model, MODELS_DIR / f"LT_{name.replace(' ', '_')}.pkl")

pred_probs = predict_probabilities(models_dict, X_test)
thresholds = compute_optimal_thresholds(pred_probs, y_test)

with open(MODELS_DIR / "LT_optimal_thresholds.json", "w") as f:
    json.dump(thresholds, f, indent=2)

results_df = build_results_table(pred_probs, thresholds, y_test)
print(results_df.to_string(index=False))
results_df.to_excel(RESULTS_DIR / "LT_bootstrap.xlsx", index=False)
